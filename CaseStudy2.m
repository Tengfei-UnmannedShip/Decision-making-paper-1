...
ship(1).evd=0.1; %本船的evidance门槛，数值越小表示对于证据距离越严谨，数值越大越宽松
ship(1).compliance=2; %compliance是对避碰规则的符合性，0：直线前进不避让，1：遵守，2：不遵守，反着来
...
ship(2).evd=0.1; %本船的evidance门槛，数值越小表示对于证据距离越严谨，数值越大越宽松
ship(2).compliance=2; %compliance是对避碰规则的符合性，0：直线前进不避让，1：遵守，2：不遵守，反着来
...
ship(3).evd=0.1; %本船的evidance门槛，数值越小表示对于证据距离越严谨，数值越大越宽松
...
ship(4).evd=0.1; %本船的evidance门槛，数值越小表示对于证据距离越严谨，数值越大越宽松
ship(4).compliance=1; %compliance是对避碰规则的符合性，0：直线前进不避让，1：遵守，2：不遵守，反着来

pos10=zeros(t,2);
pos10(1,:)=ship(1).pos;%WTF：船舶位置矩阵第一行，但是t的值尚未确定，%%猜测船舶原定最初位置
pos20=zeros(t,2);
pos20(1,:)=ship(2).pos;
pos30=zeros(t,2);
pos30(1,:)=ship(3).pos;
...
for i=2:t
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    pos10(i,:)=ship(1).pos;%WTF：扩充船舶位置矩阵，每一一个循环为一秒，每一秒就在位置矩阵下加一行，最后就是1500行。
    pos20(i,:)=ship(2).pos;
    pos30(i,:)=ship(3).pos;
    pos40(i,:)=ship(4).pos;
                ...
                if CollisionRisk(ship(OS),risk_neighbor_temp1) %WTF:本船与临船有碰撞风险时
                    ...
                    if OSdecision(OS)==1  %在第一个决策周期，不计算信息距离，CAL矩阵按照避碰理论来
                        ...
                        if ship(OS).CAL~=ship(OS).oldCAL
                            ...
                            ship(OS).OSRealdecision=[ship(OS).OSRealdecision;i 1];
                            disp('  推测的CAL有改变，重新决策');
                        else
                                ...
                                for jj=1:ShipNum  %从第一艘船到第四艘船的遍历，没有本船与目标船的区别
                                    ...
                                    if ship(OS).ship(jj).courseAlter==0 && ~isempty(neighbor_temp1)%WTF:本船没有转向决策且不是上一步已经判断出来的没有需要让路的船的直航船
                                        if CollisionRisk(ship(OS).ship(jj),neighbor_temp1) %WTF:本船与临船有碰撞风险时
                                            if OS==jj %本船自己的决策时，本船知道自己是否遵守规则

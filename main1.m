...
for i=2:t
    ...
    for OS=1:1:ShipNum  %从第一艘船到第四艘船的遍历
        if decisioncycle(OS,i,ship) %判断当前i时刻是在j船的决策周期中
            ...
            % 如果是第一个决策周期
            if i<=ship(OS).decisioncycle  %在第一个决策周期，不计算信息距离，CAL矩阵按照避碰理论来
            ...
            for ii=1:1500
                if ii==1  %当前的状态
                    ...
                    if ii==ship(OS).decisioncycle+1  %当前的状态
                        for kk = 1:ShipNum  %从第一艘船到第四艘船的遍历，没有本船与目标船的区别
                            %把下一个时刻的预测状态存上
                    ...
                    for jj=1:ShipNum  %从第一艘船到第四艘船的遍历，没有本船与目标船的区别
                        neighbor_temp1=SearchNeighbor(ship(jj),ship,ship(jj).range);%第j艘船的邻船，即进入检测范围的所有船
                        neighbor_temp2=SearchNeighbor(ship(jj),ship,5.5*1852);%WTF:第j艘船的所有风险邻船，即进入5.5海里的所有船
                        if ship(OS).ship(jj).courseAlter==0 && ~isempty(neighbor_temp1)%WTF:本船没有转向决策且不是上一步已经判断出来的没有需要让路的船的直航船
                            if CollisionRisk(ship(jj),neighbor_temp1) %WTF:本船与临船有碰撞风险时
                                [ship(OS).ship(jj).ratio,ship(OS).ship(jj).courseAlter,ship(OS).ship(jj).courseTime] = Decision_making(ship(jj),neighbor_temp2);
                        ...
                        %OS本身的决策值
                        ship(OS).data=[ship(jj).data;ii ship(jj).ratio ship(jj).courseAlter ship(jj).courseTime];
                    end
                end
                %% 返回对下一个决策周期的预测值和本身的决策值
                %OS对下一个决策周期所有船舶位置的预测值
            end
        end
    end
end

function [ Inference ] = Pre_infer1( pristate,prestate,poststate )
%% 本程序用于计算t(i+1)时隙观察到的实际船舶状态与上一时隙t预测的状态的距离；
...
%先验概率
pre_m_S_P=F0(1.5*pre_Gain_S,0.6,0.7 ); %CauchyCirve(pri_Gain_S*rou,0.6,0.7)
pre_m_S_NP=F0( 2-1.5*pre_Gain_S,0.6,0.7 );
pre_m_S_P_NP=1-pre_m_S_P-pre_m_S_NP;

pre_m_T_P=F0(1.5*pre_Gain_T,0.6,0.7 );%CauchyCirve(pri_Gain_T*rou,0.6,0.7)
pre_m_T_NP=F0( 2-1.5*pre_Gain_T,0.6,0.7 );
pre_m_T_P_NP=1-pre_m_T_P-pre_m_T_NP;

%后验概率
post_m_S_P=F0(1.5*post_Gain_S,0.6,0.7 );  %CauchyCirve(post_Gain_S*rou,0.6,0.7)
post_m_S_NP=F0( 2-1.5*post_Gain_S,0.6,0.7 );
post_m_S_P_NP=1-post_m_S_P-post_m_S_NP;

post_m_T_P=F0(1.5*post_Gain_T,0.6,0.7 );   %CauchyCirve(post_Gain_T*rou,0.6,0.7)
post_m_T_NP=F0( 2-1.5*post_Gain_T,0.6,0.7 );

...
% 形成追越局面的话，就需要有两艘船，航向一样，一艘航速大
%成功的场景1
boat=[
    0.0,  0.0,  18,    0,  1,  3,  6,  0.77, 1
    0.0,  0.0,  13,    0,  1,  4,  6,  0.77, 1
    0.0,  0.0,  16,  300,  1,  5,  6,  0.77, 1
    0.0,  0.0,  13,  135,  1,  5,  6,  0.77, 1
    ];

% %成功的场景2，是一种比场景1复杂的多的情况，可以好好保留
% boat=[
%     0.0,  0.0,  19,    0,  1,  3,  6,  0.77, 1
%     0.0,  0.0,  12,    0,  1,  4,  6,  0.77, 1
%     0.0,  0.0,  16,  300,  1,  5,  6,  0.77, 1
%     0.0,  0.0,  13,  135,  1,  5,  6,  0.77, 1
%     ];

% 初始化船舶的决策属性：1避碰规则符合性，2是否推测，即状态参数boat的5和9
shipLabel=[
    1 1
    1 1
    1 1
                    ...
                    if OSdecision(OS)==1  %在第一个决策周期，不计算信息距离，CAL矩阵按照避碰规则来
                        ...
                        for ship_i = 1:1:ShipNum
                            for ship_j = ShipNum:-1:ship_i+1 %只判断上三角，不包括对角线及以下
                                ...
                                %首先生成信息距离的计算
                                inferMatrix_temp = Pre_infer1(pristate,prestate,poststate);

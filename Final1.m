...
%% WTF:参数初始化
%基本参数设置，方便修改,分别是：
%       1~2位置、3航速、4初始航向，5避碰规则符合性，6决策周期时长，7检测范围（range），8证据距离阈值，
compliance0=[1 1 1 1];
boat=[   0,  1.5,  18,   0,  1,  3,  5,  0.8
         0.2,-0.2,  18, 230,  1,  4,  5,  0.8
         0.4, 0.3,  16, 300,  1,  5,  5,  0.8
         0.5, 0.5,  13, 135,  1,  5,  5,  0.8];
boat(:,5)=compliance0;
%具体参数设置
ShipNum=4;
t=2000;
...
%     ship(i).pos = boat(i,1:2)*1852;
    ship(i).speed = boat(i,3)*1852/3600;%WTF：航速为18海里/小时。1海里=1.852公里，因此1海里/小时=1.852公里／小时=1852/3600米/秒
    ship(i).initialCourse = boat(i,4);
    ship(i).compliance= boat(i,5); %compliance是对避碰规则的符合性，0：直线前进不避让，1：遵守，2：不遵守，反着来
    ship(i).decisioncycle=boat(i,6);
    ship(i).range=boat(i,7)*1852;  %detect range
    ship(i).no=i;  %receive information from other ships
    ship(i).evd=boat(i,8); %本船的evidance门槛，数值越小表示对于证据距离越严谨，数值越大越宽松
    ...
    ship(i).courseAlter = 0;
    ship(i).courseTime = 0;
    ship(i).ratio=1;
    ship(i).pos=[boat(i,1)-ship(i).speed*sind(ship(i).initialCourse)*0.5*t, boat(i,2)-ship(i).speed*cosd(ship(i).initialCourse)*0.5*t];
...
for i=2:t
    ...
    for OS=1:1:ShipNum  %从第一艘船到第四艘船的遍历
        if decisioncycle(OS,i,ship)&& ship(OS).compliance~=0%判断当前i时刻是在j船的决策周期中且compliance==1即本船正常
            ...
            if CollisionRisk0(ship(OS),risk_neighbor_temp1) %WTF:本船与临船有碰撞风险时
                OSdecConut(i,OS)=1;
...
for fig=1:6
    ...
    %WTF:画出船头的圆用于表示安全范围
    circle(pos1(k,:),500/1852,1);
    circle(pos2(k,:),500/1852,2);
    circle(pos3(k,:),500/1852,3);
    circle(pos4(k,:),500/1852,4);

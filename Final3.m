%% 决策程序为Decision-making0和Decision-making3，没有成功
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%1.12月21日改
...
boat=[
    0.0, 2.5,  18,   0,  1,  3,  5,  0.7896, 1
    0.2,-0.2,  18, 230,  1,  4,  5,  0.7896, 1
    0.4, 0.3,  16, 300,  1,  5,  5,  0.7896, 1
    0.5, 0.5,  13, 135,  1,  5,  5,  0.7896, 1
    ];
boat(:,5)=shipLabel(:,1);
boat(:,9)=shipLabel(:,2);
%具体参数设置
    ...
    for OS=1:1:ShipNum  %从第一艘船到第四艘船的遍历
        if decisioncycle(OS,i,ship)&& ship(OS).compliance~=0%判断当前i时刻是在j船的决策周期中且compliance==1即本船正常
            ...
            if ~isempty(risk_neighbor_temp1) && CollisionRisk0(ship(OS),risk_neighbor_temp1) %WTF:本船与临船有碰撞风险时
                ...
                if ship(OS).inferLabbel==0 %不管其他船舶，不推测；直接运行Jinfen的避碰算法
                        ...
                        for ship_i = 1:1:ShipNum
                            for ship_j = ShipNum:-1:ship_i+1 %只判断上三角，不包括对角线及以下
                                ...
                                if inferMatrix_temp(1,1)>ship(OS).evd %本船的evidance门槛，数值越小表示对于证据距离越严谨，数值越大越宽松
                                    ...
                                    ship(OS).CALchange_lable=[ship(OS).CALchange_lable;i ship_j ship_i inferMatrix_temp(1,1)];
                                end
                            end
                        end
                                    ...
                                    if ship(OS).ship(jj).courseAlter==0 && ~isempty(risk_neighbor_temp1)%WTF:本船没有转向决策且不是上一步已经判断出来的没有需要让路的船的直航船
                                        if CollisionRisk(ship(OS).ship(jj),risk_neighbor_temp1) %WTF:本船与临船有碰撞风险时
                                            if jj==OS %本船自己的决策时，本船知道自己是否遵守规则
                                                [ship(OS).ship(jj).ratio,ship(OS).ship(jj).courseAlter,ship(OS).ship(jj).courseTime] = Decision_making3(ship(OS).ship(jj),neighbor_temp2,ship(OS).CAL,ship(OS).compliance);
...
%% 汇总最危险时刻
for i=1:4
shipDismin(i)=min(ship(i).Dismin_Record(:,2));
shipDismin_time_temp=find(ship(i).Dismin_Record(:,2)<shipDismin(i)+0.1);
shipDismin_time(i)=shipDismin_time_temp(1);
end
shipDismin_value=min(shipDismin);
time_temp=find(shipDismin<shipDismin_value+0.1);
Most_danger_time=shipDismin_time(time_temp(1));
disp(['最短距离：t=',num2str(Most_danger_time),', Dis=',num2str(shipDismin_value)]);

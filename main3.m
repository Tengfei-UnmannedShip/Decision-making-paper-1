...
for i=1:1:ShipNum
    ...
    ship(i).pos1=ship(1).pos;%WTF：扩充船舶位置矩阵，每一一个循环为一秒，每一秒就在位置矩阵下加一行，最后就是1500行。
    ship(i).pos2=ship(2).pos;
    ship(i).pos3=ship(3).pos;
    ship(i).pos4=ship(4).pos;
    
    ship(i).c1=ship(1).initialCourse+ship(1).courseAlter;%WTF：扩充船舶航向矩阵，每个循环更新船舶当前航向（原航向角alpha+当前的偏转角beta），最后就是1500行。
    ship(i).c2=ship(2).initialCourse+ship(2).courseAlter;
    ship(i).c3=ship(3).initialCourse+ship(3).courseAlter;
    ship(i).c4=ship(4).initialCourse+ship(4).courseAlter;
        ...
        if decisioncycle(OS,i,ship) %判断当前i时刻是在j船的决策周期中
            ...
            if ship(OS).courseAlter==0 && ~isempty(risk_neighbor_temp1)%WTF:本船没有转向决策且不是上一步已经判断出来的没有需要让路的船的直航船
                if CollisionRisk(ship(OS),risk_neighbor_temp1) %WTF:本船与临船有碰撞风险时
                    ...
                    % 依然是所有船，没有本船与目标船的区别
                    if  ship(OS).CALchange==0 && OSdecision(OS)~=1 %判断是否CAL是否有变
                        ...
                        ship(OS).ship(1).prepos=ship(OS).pos1(i-ship(OS).decision_lable,:);
                        ship(OS).ship(2).prepos=ship(OS).pos2(i-ship(OS).decision_lable,:);
                        ship(OS).ship(3).prepos=ship(OS).pos3(i-ship(OS).decision_lable,:);
                        ship(OS).ship(4).prepos=ship(OS).pos4(i-ship(OS).decision_lable,:);
                        
                        ship(OS).ship(1).preCourse=ship(OS).c1(i-ship(OS).decision_lable);
                        ship(OS).ship(2).preCourse=ship(OS).c2(i-ship(OS).decision_lable);
                        ship(OS).ship(3).preCourse=ship(OS).c3(i-ship(OS).decision_lable);
                        ship(OS).ship(4).preCourse=ship(OS).c4(i-ship(OS).decision_lable);
                    else
                        for ii=1:1500
                            ...
                            if ii==1  %当前的状态
                                ...
                                ship(OS).pos1=[ship(OS).pos1;ship(OS).ship(1).pos];%WTF：扩充船舶位置矩阵，每一一个循环为一秒，每一秒就在位置矩阵下加一行，最后就是1500行。
                                ship(OS).pos2=[ship(OS).pos2;ship(OS).ship(2).pos];
                                ship(OS).pos3=[ship(OS).pos3;ship(OS).ship(3).pos];
                                ship(OS).pos4=[ship(OS).pos4;ship(OS).ship(4).pos];
                                
                                ship(OS).c1=[ship(OS).c1;ship(OS).ship(1).initialCourse+ship(OS).ship(1).courseAlter];%WTF：扩充船舶航向矩阵，每个循环更新船舶当前航向（原航向角alpha+当前的偏转角beta），最后就是1500行。
                                ship(OS).c2=[ship(OS).c2;ship(OS).ship(2).initialCourse+ship(OS).ship(2).courseAlter];
                                ship(OS).c3=[ship(OS).c3;ship(OS).ship(3).initialCourse+ship(OS).ship(3).courseAlter];
                                ship(OS).c4=[ship(OS).c4;ship(OS).ship(4).initialCourse+ship(OS).ship(4).courseAlter];

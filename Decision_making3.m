function [v_ratio,theta,time] = Decision_making3(OS,ship_temp,OS_CAL,OS_compliance)%本船知道自己是否遵守避碰规则
%% WTF:船舶坐标转换处理
if OS_compliance==0
    v_ratio=OS.ratio;
    ...
    if isempty(ship_giveway2)  %WTF:根据论文图6-5，{TS2} 为空集不用执行变速决策
        ...
        if OS_compliance==1
            %% OS_compliance==1时的变速决策
            ratio_temp=OS.ratio;
            speed_temp=OS.speed*0.05;
            if OS_CAL(OS.no,ship_temp(i).no)==1
        ...
        elseif OS_compliance==2
            %% OS_compliance==2时的变速决策
            ratio_temp=OS.ratio;
            speed_temp=OS.speed*0.05;
            if OS_CAL(OS.no,ship_temp(i).no)==0
                while ratio_temp>0.5 && CollisionRisk(OS,ship_giveway2)
                    ratio_temp=ratio_temp-0.05;
                    OS.speed=OS.speed-speed_temp;
                end
    ...
    if isempty(ship_giveway1) && ~CollisionRisk(OS,ship_giveway2)
        ...
        %% OS_compliance==1时的转向决策
        if OS_compliance==1
            if OS_CAL(OS.no,ship_temp(i).no)==1
                if CollisionRisk(OS,ship_giveway)
                    for theta_temp = 15:60
